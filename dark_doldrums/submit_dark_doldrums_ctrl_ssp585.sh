#!/bin/sh
#PBS -q iccp
#PBS -l walltime=24:00:00
#PBS -l nodes=1
#PBS -j oe
#PBS -N dark_doldrums_comparison
#PBS -A awiiccp

# Dark_Doldrums analysis for CTRL (1950C) and SSP585 (2080C) periods
# CTRL: 20 years (1995-2014)
# SSP585: 14 years (2086-2099)

SCRIPT_DIR="/mnt/lustre/home/awiiccp2/software/GlobalLab_hackathon_2025"
cd $SCRIPT_DIR

echo "=============================================="
echo "Dark_Doldrums CTRL vs SSP585 Comparison"
echo "=============================================="
echo "Start time: $(date)"

# Database for storing results
DB_PATH="${SCRIPT_DIR}/dark_doldrums_results/dark_doldrums_results.db"

# CTRL period: 1950C scenario, 1995-2014 (20 years)
echo ""
echo "=== Running CTRL (1950C) 1995-2014 ==="
OUTPUT_CTRL="${SCRIPT_DIR}/dark_doldrums_results/tco1279_CTRL"
mkdir -p $OUTPUT_CTRL

python3 dark_doldrums_analysis.py \
    --source tco1279 \
    --scenario 1950C \
    --start 1995 \
    --end 2014 \
    --output-dir $OUTPUT_CTRL \
    --region Germany \
    --db $DB_PATH

echo "CTRL finished: $(date)"

# SSP585 period: 2080C scenario, 2091-2099 (9 years - 3h data starts 2091)
echo ""
echo "=== Running SSP585 (2080C) 2091-2099 ==="
OUTPUT_SSP="${SCRIPT_DIR}/dark_doldrums_results/tco1279_SSP585"
mkdir -p $OUTPUT_SSP

python3 dark_doldrums_analysis.py \
    --source tco1279 \
    --scenario 2080C \
    --start 2091 \
    --end 2099 \
    --output-dir $OUTPUT_SSP \
    --region Germany \
    --db $DB_PATH

echo "SSP585 finished: $(date)"

echo ""
echo "=============================================="
echo "All done: $(date)"
echo "=============================================="
