#!/usr/bin/env python3
"""
Compute Dark_Doldrums events for Germany using ERA5 data.
...
References:
- Mockert et al. (2023): "A Brief Climatology of Dark_Doldrums Events"
- Kittel & Schill (2024): "Measuring the Dark_Doldrums"
"""
...
# Pure numpy moving average (avoids scipy for compatibility)
def uniform_filter1d(data, size, mode='nearest'):
    """Moving average filter using numpy (scipy-compatible replacement)."""
    kernel = np.ones(size) / size
    # Pad for 'nearest' mode behavior
    pad_size = size // 2
...
# Dark_Doldrums detection parameters (Mockert et al. 2023)
DUNKELFLAUTE_PARAMS = {
...
def load_era5_data_efficient(year, months=None):
    """
    Efficiently load ERA5 data for dark_doldrums calculation.
...
def _process_tco1279_month(args):
    """Worker function to process a single month of TCo1279 data (for parallel execution)."""
    year, month, base_path, grid_file, germany_mask_data = args
...
def load_tco1279_data(year, months=None, scenario='1950C', n_workers=None):
    """
    Load TCo1279-DART data for dark_doldrums calculation (parallel by month).
    
    Args:
        year: Year to load
        months: List of months (1-12) or None for full year
        scenario: '1950C' or '2080C'
        n_workers: Number of parallel workers (default: min(12, cpu_count))
    ...
    if scenario == '2080C':
        base_path = "/scratch/awicm3/TCo1279-DART-2080C/outdata/oifs"
    else:
    ...
    print(f"  Using {n_workers} parallel workers for {len(months)} months")
    ...
    with Pool(n_workers) as pool:
        month_results = pool.map(_process_tco1279_month, args_list)
    
    # Combine results
    for month, results in zip(months, month_results):
        if results:
            print(f"    Month {month:02d}: {len(results)} timesteps")
            all_results.extend(results)
        else:
            print(f"    Month {month:02d}: no data")
    
    print(f"  Total: {len(all_results)} timesteps")
    
    return pd.DataFrame(all_results).set_index('time').sort_index()
...
def detect_dark_doldrums_events(df, moving_avg_hours=48, threshold=0.06, 
                               min_duration_hours=24, timestep_hours=1):
    """
    Detect Dark_Doldrums events using the Mockert et al. (2023) methodology.
...
def compute_annual_statistics(df, events):
    """Compute annual dark_doldrums statistics."""
    stats = {
        'n_events': len(events),
...
def analyze_year(year, save_results=True, output_dir=None, data_source='era5', scenario='1950C'):
    """
    Run full dark_doldrums analysis for a single year.
    ...
    print(f"\n{'='*60}")
    print(f"Dark_Doldrums Analysis for Germany - {year}")
    print(f"{'='*60}")
    ...
    # Detect events
    print("\nDetecting Dark_Doldrums events...")
    events, df = detect_dark_doldrums_events(
        df,
    ...
    print(f"\nDark_Doldrums events: {stats['n_events']}")
    print(f"Total Dark_Doldrums hours: {stats['total_hours']}")
    ...
    if save_results and output_dir:
        ...
        # Save time series
        df.to_csv(output_dir / f"dark_doldrums_timeseries_{year}.csv")
        
        # Save events
        if events:
            events_df = pd.DataFrame(events)
            events_df.to_csv(output_dir / f"dark_doldrums_events_{year}.csv", index=False)
...
def analyze_multi_year(start_year, end_year, output_dir=None, data_source='era5', scenario='1950C',
    ...
    """
    Run dark_doldrums analysis for multiple years.
    ...
    if annual_stats:
        ...
        print(f"Total Dark_Doldrums hours: {total_hours}")
        print(f"Average events/year: {total_events/len(years):.1f}")
        print(f"Average Dark_Doldrums hours/year: {total_hours/len(years):.1f}")
    ...
    if output_dir:
        ...
        stats_df.to_csv(output_dir / f"dark_doldrums_annual_stats_{start_year}_{end_year}.csv")
        
        # Save all events
        if all_events:
            events_df = pd.DataFrame(all_events)
            events_df.to_csv(output_dir / f"dark_doldrums_all_events_{start_year}_{end_year}.csv", index=False)
    
    # Save to database
    if db_path and annual_stats:
        from dark_doldrums_db import save_results_to_db
...
def main():
    parser = argparse.ArgumentParser(
        description='Compute Dark_Doldrums events for Germany using ERA5 or TCo1279-DART data',
...
Examples:
  ...
  %(prog)s --source tco1279 --scenario 2080C   # Use 2080C scenario
    ...
    parser.add_argument('--threshold', type=float, default=0.06,
                        help='Dark_Doldrums threshold (default: 0.06)')
    parser.add_argument('--moving-avg', type=int, default=48,
                        help='Moving average window in hours (default: 48)')
    parser.add_argument('--output-dir', type=str, 
                        default='/work/ab0246/a270092/software/GlobalLab_hackathon_2025/dark_doldrums_results',
