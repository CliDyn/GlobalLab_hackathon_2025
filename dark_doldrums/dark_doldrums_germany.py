#!/usr/bin/env python3
"""
Compute longest Dark_Doldrums events over Germany.

Dark_Doldrums = sustained periods with low combined wind + solar power generation.
...
Key insight: Duration matters! A 3-hour dip is not a Dark_Doldrums.
...
# Dark_Doldrums thresholds (absolute capacity factors, as in Li et al.)
...
def find_dark_doldrums_events(wind_cf, solar_cf, min_duration_hours, timestep_hours=3):
    """
    Find Dark_Doldrums events: sustained periods where BOTH wind AND solar are low.
    ...
    # Dark_Doldrums: BOTH wind AND solar below thresholds (sustained)
...
def main():
    parser = argparse.ArgumentParser(description='Compute Dark_Doldrums events over Germany')
    ...
    print(f"{'='*60}")
    print(f"Dark_Doldrums Analysis for Germany")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"Dark_Doldrums Events (sustained low wind AND solar)")
    print(f"{'='*60}")
    
    for min_dur in MIN_DURATIONS:
        events = find_dark_doldrums_events(wind_cf, solar_cf, min_dur)
